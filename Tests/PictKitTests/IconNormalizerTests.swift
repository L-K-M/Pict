...
final class IconNormalizerTests: XCTestCase {
    ...
    func testBleedIsClampedToTheSupportedRange() {
        ...
        XCTAssertEqual(IconNormalizer.clampedBleed(9), IconNormalizer.maximumBleed)
        XCTAssertEqual(IconNormalizer.clampedBleed(.nan), 0)
    }

    /// The agreement between this canvas and Zap's `IconRowMetrics.imageExtent`
    /// is asserted on Zap's side, where both halves are visible. If they diverge
    /// the cached bitmap stops filling the frame it is drawn into.
