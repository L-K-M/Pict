...
/// Matching a Mac app to an icon in a Linux theme (Pict's `UNJAILED.md`).
///
...
final class IconNameGuessTests: XCTestCase {
    ...
    /// The row Pict's `UNJAILED.md` marks ❌: close, and wrong without help.
    ...
    func testAnUnknownAppSuggestsNothing() {
        let matches = IconNameGuess.matches(appName: "Pict",
                                            bundleIdentifier: "ch.lkmc.Pict", in: theme)
