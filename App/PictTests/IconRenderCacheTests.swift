...
final class IconRenderCacheTests: XCTestCase {
    ...
    func testKeysAreAlwaysSafeFileNames() {
        for source in [markup(), Data("../../escape".utf8), Data(), Data([0xFF, 0x00])] {
            XCTAssertTrue(IconEntryKey.isSafeFileName(IconRenderCache.key(for: source, side: 64), extensions: ["png"]))
        }
    }
    ...
    /// A truncated or half-written file is Pict's own output gone wrong, not hostile
    ...
    func testACorruptEntryIsDiscardedRatherThanReturned() throws {
        ...
        let stored = try XCTUnwrap(
            IconEntryKey.resolvedURL(forFile: IconRenderCache.key(for: markup(), side: 128),
                                     in: directory, extensions: ["png"]))
