...
/// An installed theme as something the search sheet can search (Pict's `UNJAILED.md`).
final class IconSetProviderTests: XCTestCase {
    ...
    func testSuggestionsAreEmptyForAnAppTheSetDoesntCover() async {
        let suggestions = await makeProvider().suggestions(
            appName: "Pict", bundleIdentifier: "ch.lkmc.Pict", limit: 5)
    ...
    /// The set was searched, then emptied behind Pict's back. Nothing went over a
