...
/// Finds an icon for one app without leaving Pict (Zap's `UNJAILED.md`, §5.5, §5.6).
///
...
struct IconSearchSheet: View {
    ...
    init(row: IconTargetRow, providers: [IconSearchProvider], preferences: PictPreferences,
        ...
        _query = State(initialValue: row.name)
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Find an icon for \(row.name)")
                .font(.headline)
    ...
    private var disclosure: some View {
        VStack(alignment: .leading, spacing: 10) {
            Label("Before Pict searches", systemImage: "hand.raised")
    ...
    private func loadSuggestions() {
        ...
        let source = provider
        Task {
            let found = await source.suggestions(appName: row.name,
