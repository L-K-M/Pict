...
/// Owns `~/Library/Application Support/Pict/IconSets` — the themes the user has
/// installed, and what Pict knows about each one.
///
...
final class IconSetLibrary: ObservableObject {
    ...
    static func defaultDirectory() -> URL {
        ...
        return base.appendingPathComponent("Pict/IconSets", isDirectory: true)
    }
    ...
    /// Set identifiers are Pict's own, from a compile-time catalogue, so they can't
    ...
    /// Pict's own PNGs at adoption time (`IconStore`), so nothing here is load-bearing
    ...
    /// (or its directory has been emptied behind Pict's back).
    ///
    ...
    private func saveRecords() {
        do {
            ...
            NSLog("Pict: couldn't save the icon set index: %@", error.localizedDescription)
        }
    }
