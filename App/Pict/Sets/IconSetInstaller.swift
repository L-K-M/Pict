...
/// Downloads an icon theme, keeps the part Pict needs, and throws the rest away.
///
...
/// larger than anything Pict should hold.
///
/// **This spawns `/usr/bin/tar`, which nothing else in Pict does.** Foundation has no
...
enum IconSetInstaller {
    ...
    enum InstallError: Error, Equatable {
        ...
        var message: String {
            switch self {
            case .badURL:
                return "That icon set's address isn't something Pict can fetch."
            case .download(let reason):
                return "Pict couldn't download that icon set. \(reason)"
            case .extraction(let reason):
                return "Pict couldn't unpack that icon set. \(reason)"
            case .nothingMatched:
                return "That icon set didn't contain any icons where Pict expected them. "
    ...
    private static func extractSynchronously(archive: URL, matching glob: String,
        ...
        guard process.terminationStatus == 0 else {
            ...
            NSLog("Pict: tar failed extracting an icon set: %@", reason)
