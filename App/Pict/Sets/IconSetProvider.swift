...
/// same reason. `PLAN.md §2` asks for the corpus to be local precisely so
...
struct IconSetProvider: IconSearchProvider {
    ...
    func fetchImageData(for result: IconSearchResult) async -> Result<Data, IconSearchError> {
        ...
        // Bounded like any other source, even though Pict unpacked this file itself:
        // the limit is about what gets decoded, and the archive it came out of was
        // downloaded (`PLAN.md §2`).
        let size = (try? url.resourceValues(forKeys: [.fileSizeKey]))?.fileSize ?? 0
        guard size <= IconImageValidator.Limits.maximumSourceBytes else {
            return .failure(.unavailable("That icon is larger than Pict will read."))
        }
        do {
    ...
    ///
    /// The same bounds check `IconEntryKey.resolvedURL(forFile:in:extensions:)` does, and for a
