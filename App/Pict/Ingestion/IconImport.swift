...
/// answer "whatever ImageIO decodes" the moment SVG arrived (Zap's `UNJAILED.md`,
/// §6.3). SVG is markup, not a container ImageIO has ever read, and it reaches Pict
...
/// is what keeps those paths agreeing about what Pict accepts — a second ingestion
/// path that skipped this is precisely how SVG came to be refused with "that file
/// isn't an image Pict can read".
///
...
enum IconImport {
    ...
    /// standing up a web view — the render is on Zap's `UNJAILED.md`'s manual list,
    ...
    /// Pict stores, and the search sheet asks for something much smaller for its
    ...
    async -> Result<CGImage, IconImageValidator.Rejection> {
        ...
        case .bitmap:
            return IconImageValidator.decode(data: data)
                .map { IconBitmap.downsample($0, longestEdge: side) }
        case .svg:
