...
///
/// What lands on disk is Pict's own PNG output, exactly as in `IconStore`, so
/// nothing of the source container survives here either (`PLAN.md §2`) —
...
struct IconRenderCache {
    ...
    /// Pict will fetch again anyway. So this doesn't belong in a backup, and the
    ...
    static func defaultDirectory() -> URL {
        ...
        return base.appendingPathComponent("Pict/RenderCache", isDirectory: true)
    }

    // MARK: Key

    /// How the renderer behaves, not what version Pict is.
    ///
    ...
    func image(for data: Data, side: Int) -> CGImage? {
        ...
        case .success(let image):
            return image
        case .failure:
            // Unreadable means truncated or corrupt, not malicious — this is Pict's
    ...
    func store(_ image: CGImage, for data: Data, side: Int) {
        ...
        do {
            ...
            NSLog("Pict: couldn't create the render cache directory: %@", error.localizedDescription)
            return
        }
    ...
    private func url(for data: Data, side: Int) -> URL? {
        ...
        IconEntryKey.resolvedURL(forFile: Self.key(for: data, side: side), in: directory, extensions: ["png"])
    }
