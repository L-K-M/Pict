...
final class SVGRasterizer: NSObject, WKNavigationDelegate {
    ...
    enum RasterizeError: Error, Equatable {
        ...
        var message: String {
            ...
            case .snapshotFailed: return "Pict couldn't render that SVG."
            }
        }
    }
    ...
    async -> Result<CGImage, RasterizeError> {
        ...
        guard let composed = alphaRecovered(black: onBlack, white: onWhite) else {
            NSLog("Pict: SVG alpha recovery failed")
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        ...
        webView.createPDF(configuration: configuration) { [weak self] result in
            ...
            case .failure(let error):
                ...
                NSLog("Pict: SVG PDF render failed: %@", error.localizedDescription)
                self.finish(.failure(.snapshotFailed))
            case .success(let data):
                guard let image = Self.image(fromPDF: data, side: self.side) else {
                    NSLog("Pict: SVG PDF had no drawable page")
                    self.finish(.failure(.snapshotFailed))
                    return
                }
    ...
    ///
    /// The transparency is this function's, not WebKit's: `IconBitmap.makeContext`
    ...
    static func image(fromPDF data: Data, side: Int) -> CGImage? {
        guard let provider = CGDataProvider(data: data as CFData),
              ...
              let context = IconBitmap.makeContext(width: side, height: side) else { return nil }
    ...
    func webView(_ webView: WKWebView, didFail navigation: WKNavigation!, withError error: Error) {
        NSLog("Pict: SVG navigation failed: %@", error.localizedDescription)
        finish(.failure(.snapshotFailed))
    }

    func webView(_ webView: WKWebView, didFailProvisionalNavigation navigation: WKNavigation!,
                 withError error: Error) {
        NSLog("Pict: SVG load failed: %@", error.localizedDescription)
        finish(.failure(.snapshotFailed))
    }
}
