...
enum IconSearchError: Error, Equatable {
    ...
    var message: String {
        ...
        case .decoding: return "That provider returned something Pict couldn't read."
...
/// A source of icons Pict can search.
///
/// The protocol exists mostly to keep the door open: Zap's `UNJAILED.md` concludes
...
protocol IconSearchProvider {
    ...
    /// the first search of a session (`PLAN.md §2`).
    ///
    ...
    /// (`PLAN.md §2`: "the app's name should surface its likely icon first").
    ///
