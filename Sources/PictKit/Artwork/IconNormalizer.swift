...
public enum IconNormalizer {
    ...
    public static let referenceInkFraction = 0.95
    ...
    public static let trimResolution = 256

    /// The widest bleed any caller may request, as a fraction of the nominal frame.
    ///
    /// Lived on Zap's `IconRowMetrics` while the switcher was the only consumer, and
    /// moved here when it stopped being: a dock tile and a drawer slot have their own
    /// geometry, and none of them should be reaching into another app's row metrics
    /// to find out how far artwork may spill. `IconEntry.maximumBleed` matches it, so
    /// a stored per-target override and a live request clamp to the same ceiling.
    ...
    public static func layout(inkFraction: Double, trimmedWidth: Double, trimmedHeight: Double,
    ...
    /// each side. Zap's `IconRowMetrics.imageExtent(iconSize:bleed:)` mirrors this;
    /// the two must agree or the row's cell will not contain what is drawn into it.
    public static func canvasExtent(targetExtent: Double, bleed: Double) -> Double {
        targetExtent * (1 + 2 * clampedBleed(bleed))
    }

    public static func clampedBleed(_ bleed: Double) -> Double {
    ...
    public static func normalize(_ image: CGImage, targetExtent: Int, bleed: Double,
